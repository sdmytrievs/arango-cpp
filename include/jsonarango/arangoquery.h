...
using  FetchingDocumentCallback = std::function<void( std::string&& jsondata )>;
/// Callback function fetching document and _id from a collection that match the specified condition
using  FetchingDocumentIdCallback = std::function<void( std::string&& jsondata, std::string&& id )>;
